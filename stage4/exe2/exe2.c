...
void printGsymbol(void)
{
    ...
    while (temp != NULL)
    {
        if (temp->type == TYPE_INT)
            ...
            printf("%s\tINT_PTR\t%d\t%d\t%d\t%d\t%d\n",
                   temp->name, temp->size, temp->rows, temp->columns,
                   temp->dimension, temp->binding);

        else if (temp->type == TYPE_STR_PTR)
            printf("%s\tSTR_PTR\t%d\t%d\t%d\t%d\t%d\n",
                   temp->name, temp->size, temp->rows, temp->columns,
                   temp->dimension, temp->binding);
        ...
        case NODE_ID:
            ...
            if (root->Gentry != NULL)
            {
                ...
                if (root->Gentry->type == TYPE_INT)
                    ...
                    printf("STR");
                else if (root->Gentry->type == TYPE_INT_PTR)
                    printf("INT_PTR");
                else if (root->Gentry->type == TYPE_STR_PTR)
                    printf("STR_PTR");
...
int codeGen(tnode *t, FILE *target_file)
{
    ...
    {
        reg = codeGen(t->left, target_file);
    ...
    if (t->nodetype == NODE_ASSIGN)
    {
        ...
        {
            int pointerreg = codeGen(t->left->left, target_file);
            fprintf(target_file, "MOV [R%d], R%d\n", pointerreg, i);
            freeReg(pointerreg);
        }
