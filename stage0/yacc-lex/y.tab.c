...
{
  "\"end of file\"", "error", "\"invalid token\"", "ID", "NEWLINE", "'+'",
...
#line 45 "q3.y"
        {
            size_t len = 2 + 1 + strlen((yyvsp[-2].str)) + 1 + strlen((yyvsp[0].str)) + 1;
            (yyval.str) = (char*)malloc(len);
            if(!(yyval.str)) { perror("malloc"); exit(1); }
